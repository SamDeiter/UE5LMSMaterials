...
export class LayoutController {
  ...
  initResizers() {
    const attach = (resizer, type) => {
      ...
      resizer.addEventListener("mousedown", (e) => {
        ...
        document.body.style.cursor =
          type === "left" || type === "right" ? "col-resize" : "row-resize";
    ...
    // Stats panel resizer removed - UE5 Stats panel is fixed-height, not resizable
  ...
  handleMouseMove(e) {
    ...
    if (this.currentResizer === "left") {
      ...
      this.leftWidth = Math.max(
        ...
        Math.min(LAYOUT.MAX_PANEL_WIDTH, this.startSize + delta),
      );
  ...
  updateLayout() {
    ...
    const resizeViewport =
      this.currentResizer === "viewport" || this.currentResizer === "stats";
