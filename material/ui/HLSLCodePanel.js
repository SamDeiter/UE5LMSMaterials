...
export class HLSLCodePanel {
  ...
  init() {
    ...
    // Bind close button
    header.querySelector('.hlsl-close-btn').addEventListener('click', () => this.hide());
  ...
  generateHLSL() {
    ...
    const nodes = [...material.nodes.values()].filter(n => n.type !== 'main-output');
  ...
  generateMainFunction(material) {
    ...
    code += '    PixelMaterialInputs.BaseColor = float3(1.0, 1.0, 1.0);  // White\n';
    code += '    PixelMaterialInputs.Metallic = 0.0;                      // Dielectric\n';
    code += '    PixelMaterialInputs.Specular = 0.5;                      // 4% F0\n';
    code += '    PixelMaterialInputs.Roughness = 0.5;                     // Mid-rough\n';
    code += '    PixelMaterialInputs.EmissiveColor = float3(0, 0, 0);     // No emission\n';
    code += '    PixelMaterialInputs.Opacity = 1.0;                       // Opaque\n';
    code += '    PixelMaterialInputs.Normal = float3(0, 0, 1);            // Up\n';
    code += '    PixelMaterialInputs.AmbientOcclusion = 1.0;              // No occlusion\n';
    code += '\n';
    
    // Add Cook-Torrance BRDF comment
    code += '    // Cook-Torrance Microfacet BRDF:\n';
    code += '    // f_r(l, v) = D(h) * F(v,h) * G(l,v,h) / (4 * (N.L) * (N.V))\n';
    code += '    //\n';
    code += '    // D = GGX/Trowbridge-Reitz: α² / (π * ((N.H)² * (α² - 1) + 1)²)\n';
