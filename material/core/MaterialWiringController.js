...
export class MaterialWiringController {
  ...
  drawWire(link) {
    const outputDot =
      link.sourcePin.element.querySelector(".pin-dot") ||
      link.sourcePin.element;
    const inputDot =
      link.targetPin.element.querySelector(".pin-dot") ||
      link.targetPin.element;

    if (!outputDot || !inputDot) return;

    const graphRect = this.graphPanel.getBoundingClientRect();

    // Get bounding rects for pin dots
    let outRect = outputDot.getBoundingClientRect();
    let inRect = inputDot.getBoundingClientRect();

    // Handle collapsed nodes - if pin dot has zero size, use node header instead
    if (outRect.width === 0 || outRect.height === 0) {
      const sourceNode = link.sourcePin.node?.element;
      if (sourceNode) {
        const nodeRect = sourceNode.getBoundingClientRect();
        // Output pins should connect from the right side of the header
        outRect = {
          left: nodeRect.right - 5,
          top: nodeRect.top + 12,
          width: 10,
          height: 10,
        };
    ...
    if (inRect.width === 0 || inRect.height === 0) {
      const targetNode = link.targetPin.node?.element;
      if (targetNode) {
        const nodeRect = targetNode.getBoundingClientRect();
        // Input pins should connect from the left side of the header
        inRect = {
          left: nodeRect.left - 5,
          top: nodeRect.top + 12,
          width: 10,
          height: 10,
        };
